...
ggplot() +
  ...
  geom_sf_text(data = schools$osm_polygons, aes(label = name), size = 1, hjust = 0) +
  coord_sf(xlim = c(-124.48200, -114.1308), ylim = c(32.52952,   42.0095), expand = FALSE) +
  theme_minimal()
```
...
```{r}
#retrieving data of streets in California
streets <- bb %>%
  opq() %>%
...
ggplot() +
  ...
  geom_sf(data = schools$osm_polygons, inherit.aes = FALSE, colour = "red", fill = "red", alpha = .5, size = 0.1) +
  coord_sf(xlim = c(-124.48200, -114.1308), ylim = c(32.52952,   42.0095), expand = TRUE) +
  geom_sf_text(data = schools$osm_polygons, aes(label = name), size = 2, hjust = 0) +
  theme_bw() +
  labs(
    title = "Schools in California",
    x = "Longitude",
    y = "Latitude"
  )
```
